    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if 'transportation' in unit and all(x in unit['transportation'].lower() for x in ['flight', 'departure', 'arrival']):
            ...
            for info in transport_info:
                if 'Departure Time' in info or 'Departure time' in info:
                    departure_time = info.split(':',1)[1].strip()
                elif 'Arrival Time' in info or 'Arrival time' in info:
                    arrival_time = info.split(':',1)[1].strip()
...
def is_valid_information_in_current_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if 'from' in current_city:
            ...
            final_city_list.append(extract_before_parenthesis(current_city))

        # print("[FINAL CITY LIST]", final_city_list)
...
def evaluation(query_data, tested_data):
    ...
    print("[EVAL] Reasonable visiting city done.,", is_reasonable_visiting_city(query_data, tested_data))
    ...
    print("[DEBUG]",return_info['is_valid_information_in_sandbox'])
