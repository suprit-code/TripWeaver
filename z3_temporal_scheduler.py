...
def add_accommodation_constraints(opt, days, cities, accommodation_data,
    ...
    for d in range(days):
        ...
        if d != 0:
            ...
            preferred_start_time = day_offset + windows["accommodation_start_of_day"][0]

            opt.add(s_start >= preferred_start_time)
        ...
        if d != days - 1:
            ...
            actual_sleep_duration = e_end - s_end
            preferred_sleep = durations["accommodation_end_of_day"]

            opt.add(actual_sleep_duration >= preferred_sleep)
...
if __name__ == "__main__":
    BASE_PATH = Path("output") / "5d" / "qwen_nl"       #change
    days = 5                                            #change
